...
export function activate(context: vscode.ExtensionContext) {
    ...
    let makeReadOnlyDisposable = vscode.commands.registerCommand('obscuro.makeReadOnly', async (uri: vscode.Uri) => {
        if (!uri) {
            vscode.window.showErrorMessage('No file selected.');
            return;
        }
    ...
    let makeWritableDisposable = vscode.commands.registerCommand('obscuro.makeWritable', async (uri: vscode.Uri) => {
        if (!uri) {
            vscode.window.showErrorMessage('No file selected.');
            return;
        }
...
async function handleObscure(targetPath: string, action: 'encrypt' | 'decrypt', secretManager: SecretManager) {
                    ...
                    if (stored && stored === password) {
                        ...
                        if (retry === "Enter Password") {
                            ...
                            if (newPassword) {
                                ...
                                if (save === 'Yes') await secretManager.storePassword(targetPath, newPassword);
                                return;
                            }
                        }
                    }
